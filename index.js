...
while (row * board_width < room_height) {
    ...
    var row_length = this_length;
    var row_offsets = [ this_length ];
    ...
    var row_rest_length = row_length - room_width;
    var last_board_length = board_length - row_rest_length;
    row_offsets.push(last_board_length);

    // Add * to the label if you need to throw away the remaining piece
    console.log("[" + row_offsets.join(" ") +"] (" + series_label[series_idx] +
        ((row_rest_length < min_rest_length) ? "*" : "")+ ") row");
...
var all_boards_total_width = row * board_width;
var extra_width = all_boards_total_width - room_height;
var last_row_actual_width = board_width - extra_width;
